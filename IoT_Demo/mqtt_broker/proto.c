#include "../include/mqtt_inc/proto.h"

#include "../include/mqtt_inc/ringbuf_mqtt.h"
...
I16 ICACHE_FLASH_ATTR PROTO_AddRb(RINGBUF * rb, const U8 * packet, I16 len) {
	...
	case 0x7E:
	case 0x7F:
	    if (ring_buffer_put(rb, 0x7D) == -1)
		return -1;
	    if (ring_buffer_put(rb, *packet++ ^ 0x20) == -1)
		return -1;
	    i += 2;
	    break;
	default:
	    if (ring_buffer_put(rb, *packet++) == -1)
		return -1;
	    i++;
	    break;
	}
    }
    if (ring_buffer_put(rb, 0x7F) == -1)
	return -1;
