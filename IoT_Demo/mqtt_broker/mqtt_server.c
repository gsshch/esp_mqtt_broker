#include "mqtt_inc/mqtt_server.h"

#include "user_interface.h"
#include "mem.h"

#include "mqtt_inc/debug.h"
#include "mqtt_inc/mqtt_retainedlist.h"
#include "mqtt_inc/mqtt_topiclist.h"
#include "mqtt_inc/mqtt_topics.h"
...
bool ICACHE_FLASH_ATTR publish_topic(topic_entry * topic_e, uint8_t * topic,
	...
	clientcon->mqtt_state.outbound_message = mqtt_msg_publish(
			&clientcon->mqtt_state.mqtt_connection, topic, data, data_len,
			topic_e->qos, 0, &message_id);
...
bool ICACHE_FLASH_ATTR publish_retainedtopic(retained_entry * entry,
		void* user_data) {
	uint16_t message_id = 0;
	...
	clientcon->mqtt_state.outbound_message = mqtt_msg_publish(
			&clientcon->mqtt_state.mqtt_connection, entry->topic, entry->data,
			entry->data_len, entry->qos, 1, &message_id);
	if (queue_puts(&clientcon->msgQueue,
			clientcon->mqtt_state.outbound_message->data,
			clientcon->mqtt_state.outbound_message->length) == -1) {
		MQTT_ERROR("MQTT: Queue full\r\n");
...
bool ICACHE_FLASH_ATTR activate_next_client() {
	...
	for (clientcon = clientcon_list; clientcon != NULL ; clientcon = clientcon->next) {
		if ((!queue_is_empty(&clientcon->msgQueue)) && clientcon->pCon->state != ESPCONN_CLOSE) {
...
bool ICACHE_FLASH_ATTR mqtt_server_init_client_connect(MQTT_ClientCon * mqttClientCon) {
	uint32_t temp;
	...
	mqtt_msg_init(&mqttClientCon->mqtt_state.mqtt_connection,
			mqttClientCon->mqtt_state.out_buffer,
...
bool ICACHE_FLASH_ATTR mqtt_server_deleteClientCon(
	...
	if (mqttClientCon->connect_info.will_topic != NULL) {
		...
		find_topic(mqttClientCon->connect_info.will_topic, publish_topic,
				mqttClientCon->connect_info.will_data,
				mqttClientCon->connect_info.will_data_len);
		activate_next_client();

		if (mqttClientCon->connect_info.will_retain) {
			update_retainedtopic(mqttClientCon->connect_info.will_topic,
					mqttClientCon->connect_info.will_data,
					mqttClientCon->connect_info.will_data_len,
					mqttClientCon->connect_info.will_qos);
		}
...
void ICACHE_FLASH_ATTR mqtt_server_disconnectClientCon(
		MQTT_ClientCon * mqttClientCon) {
...
static void ICACHE_FLASH_ATTR mqtt_client_connect_recv_cb(void *arg, char *pdata, unsigned short len) {
	uint8_t msg_type;
	uint8_t msg_qos;
	uint16_t msg_id;
	...
	READPACKET:
	os_memcpy(
			&clientcon->mqtt_state.in_buffer[clientcon->mqtt_state.message_length_read],
			pdata, len);
	clientcon->mqtt_state.message_length_read += len;

	clientcon->mqtt_state.message_length = mqtt_get_total_length(
			clientcon->mqtt_state.in_buffer,
			clientcon->mqtt_state.message_length_read);
	MQTT_INFO("MQTT: total_len: %d\r\n", clientcon->mqtt_state.message_length);
	if (clientcon->mqtt_state.message_length
			> clientcon->mqtt_state.message_length_read) {
	...
	case TCP_CONNECTED:
		...
		case MQTT_MSG_TYPE_CONNECT:
			...
			if (clientcon->mqtt_state.message_length
					< sizeof(struct mqtt_connect_variable_header) + 3) {
			...
			uint16_t var_header_len =
					sizeof(struct mqtt_connect_variable_header4);

			// We check MQTT v3.11 (version 4)
			if ((variable_header->lengthMsb << 8) + variable_header->lengthLsb
					== 4 && variable_header->version == 4
				...
				if ((variable_header3->lengthMsb << 8)
						+ variable_header3->lengthLsb == 6
						&& variable_header3->version == 3
						&& os_strncmp(variable_header3->magic, "MQIsdp", 6)
								== 0) {
					...
					variable_header->keepaliveMsb =
							variable_header3->keepaliveMsb;
					variable_header->keepaliveLsb =
							variable_header3->keepaliveLsb;
				} else {
					// Neither found
			...
			clientcon->connect_info.clean_session = (variable_header->flags
					& MQTT_CONNECT_FLAG_CLEAN_SESSION) != 0;

			clientcon->connect_info.keepalive = (variable_header->keepaliveMsb
					<< 8) + variable_header->keepaliveLsb;
			espconn_regist_time(clientcon->pCon,
					2 * clientcon->connect_info.keepalive, 1);
			MQTT_INFO("MQTT: Keepalive %d\r\n", clientcon->connect_info.keepalive);

			// Get the client id
			uint16_t id_len = clientcon->mqtt_state.message_length
					- (2 + msg_used_len);
			const char *client_id = mqtt_get_str(
					&clientcon->mqtt_state.in_buffer[2 + msg_used_len],
					&id_len);
			...
			if (id_len == 0) {
				...
				if (!clientcon->connect_info.clean_session) {
					MQTT_WARNING(
							"MQTT: Null client Id and NOT cleansession\r\n");
					msg_conn_ret = CONNECTION_REFUSE_ID_REJECTED;
					clientcon->connState = TCP_DISCONNECTING;
					break;
				}
			...
			// Get the LWT
			clientcon->connect_info.will_retain = (variable_header->flags
					& MQTT_CONNECT_FLAG_WILL_RETAIN) != 0;
			clientcon->connect_info.will_qos = (variable_header->flags & 0x18)
					>> 3;
			if (!(variable_header->flags & MQTT_CONNECT_FLAG_WILL)) {
				// Must be all 0 if no lwt is given
				if (clientcon->connect_info.will_retain
						|| clientcon->connect_info.will_qos) {
					MQTT_WARNING("MQTT: Last Will flags invalid\r\n");
					mqtt_server_disconnectClientCon(clientcon);
					return;
				}
			} else {
				uint16_t lw_topic_len = clientcon->mqtt_state.message_length
						- (2 + msg_used_len);
				const char *lw_topic = mqtt_get_str(
						&clientcon->mqtt_state.in_buffer[2 + msg_used_len],
						&lw_topic_len);
				...
				clientcon->connect_info.will_topic = (char *) os_zalloc(
						lw_topic_len + 1);
				if (clientcon->connect_info.will_topic != NULL) {
					os_memcpy(clientcon->connect_info.will_topic, lw_topic,
							lw_topic_len);
					clientcon->connect_info.will_topic[lw_topic_len] = 0;
					if (Topics_hasWildcards(
					...
					if (clientcon->connect_info.will_topic[0] == '$') {
						MQTT_WARNING(
								"MQTT: Last Will topic starts with '$'\r\n");
						mqtt_server_disconnectClientCon(clientcon);
						return;
				...
				uint16_t lw_data_len = clientcon->mqtt_state.message_length
						- (2 + msg_used_len);
				const char *lw_data = mqtt_get_str(
						&clientcon->mqtt_state.in_buffer[2 + msg_used_len],
						&lw_data_len);
				...
				clientcon->connect_info.will_data = (char *) os_zalloc(
						lw_data_len);
				clientcon->connect_info.will_data_len = lw_data_len;
				if (clientcon->connect_info.will_data != NULL) {
					os_memcpy(clientcon->connect_info.will_data, lw_data,
							lw_data_len);
			...
			if ((variable_header->flags & MQTT_CONNECT_FLAG_USERNAME) != 0) {
				uint16_t username_len = clientcon->mqtt_state.message_length
						- (2 + msg_used_len);
				const char *username = mqtt_get_str(
						&clientcon->mqtt_state.in_buffer[2 + msg_used_len],
						&username_len);
				...
				clientcon->connect_info.username = (char *) os_zalloc(
						username_len + 1);
				if (clientcon->connect_info.username != NULL) {
					os_memcpy(clientcon->connect_info.username, username,
							username_len);
			...
			if ((variable_header->flags & MQTT_CONNECT_FLAG_PASSWORD) != 0) {
				...
				if ((variable_header->flags & MQTT_CONNECT_FLAG_USERNAME)
						== 0) {
				...
				uint16_t password_len = clientcon->mqtt_state.message_length
						- (2 + msg_used_len);
				const char *password = mqtt_get_str(
						&clientcon->mqtt_state.in_buffer[2 + msg_used_len],
						&password_len);
				...
				clientcon->connect_info.password = (char *) os_zalloc(
						password_len + 1);
				if (clientcon->connect_info.password != NULL) {
					os_memcpy(clientcon->connect_info.password, password,
							password_len);
			...
			// Check Auth
			if ((local_auth_cb != NULL )&& local_auth_cb(
							clientcon->connect_info.username == NULL ?
									"" : clientcon->connect_info.username,
							clientcon->connect_info.password == NULL ?
									"" : clientcon->connect_info.password,
		...
		clientcon->mqtt_state.outbound_message = mqtt_msg_connack(
				&clientcon->mqtt_state.mqtt_connection, msg_conn_ret);
		if (queue_puts(&clientcon->msgQueue,
				clientcon->mqtt_state.outbound_message->data,
				clientcon->mqtt_state.outbound_message->length) == -1) {
			MQTT_ERROR("MQTT: Queue full\r\n");
	...
	case MQTT_MSG_TYPE_SUBSCRIBE:
		...
		msg_id = mqtt_get_id(clientcon->mqtt_state.in_buffer,
				clientcon->mqtt_state.in_buffer_length);
		MQTT_INFO("MQTT: Message id %d\r\n", msg_id);
		topic_index = 4;
		num_subs = 0;
		while (topic_index < clientcon->mqtt_state.message_length
				&& num_subs < MAX_SUBS_PER_REQ) {
			topic_len = clientcon->mqtt_state.message_length - topic_index;
			topic_str = mqtt_get_str(
					&clientcon->mqtt_state.in_buffer[topic_index], &topic_len);
			if (topic_str == NULL) {
			...
			ret_codes[num_subs++] =
					add_topic(clientcon, topic_buffer, 0) ? 0 : 0x80;
			//iterate_topics(print_topic, 0);
		}
		MQTT_INFO("MQTT: Subscribe successful\r\n");

		clientcon->mqtt_state.outbound_message = mqtt_msg_suback(
				&clientcon->mqtt_state.mqtt_connection, ret_codes, num_subs,
				msg_id);
		if (queue_puts(&clientcon->msgQueue,
				clientcon->mqtt_state.outbound_message->data,
				clientcon->mqtt_state.outbound_message->length) == -1) {
	...
	case MQTT_MSG_TYPE_UNSUBSCRIBE:
		...
		topic_index = 4;
		while (topic_index < clientcon->mqtt_state.message_length) {
			uint16_t topic_len = clientcon->mqtt_state.message_length
					- topic_index;
			char *topic_str = mqtt_get_str(
					&clientcon->mqtt_state.in_buffer[topic_index], &topic_len);
			if (topic_str == NULL) {
				MQTT_WARNING("MQTT: Subscribe topic invalid\r\n");
				mqtt_server_disconnectClientCon(clientcon);
		...
		clientcon->mqtt_state.outbound_message = mqtt_msg_unsuback(
				&clientcon->mqtt_state.mqtt_connection, msg_id);
		if (queue_puts(&clientcon->msgQueue,
				clientcon->mqtt_state.outbound_message->data,
				clientcon->mqtt_state.outbound_message->length) == -1) {
			MQTT_ERROR("MQTT: Queue full\r\n");
	...
	case MQTT_MSG_TYPE_PUBLISH:
		...
		topic_str = (uint8_t *) mqtt_get_publish_topic(
				clientcon->mqtt_state.in_buffer, &topic_len);
		os_memcpy(topic_buffer, topic_str, topic_len);
		topic_buffer[topic_len] = 0;
		data_len = clientcon->mqtt_state.in_buffer_length;
		data = (uint8_t *) mqtt_get_publish_data(
				clientcon->mqtt_state.in_buffer, &data_len);
		...
		if (mqtt_get_retain(clientcon->mqtt_state.in_buffer)) {
			update_retainedtopic(topic_buffer, data, data_len,
					mqtt_get_qos(clientcon->mqtt_state.in_buffer));
		}
	...
	case MQTT_MSG_TYPE_PINGREQ:
		...
		clientcon->mqtt_state.outbound_message = mqtt_msg_pingresp(
				&clientcon->mqtt_state.mqtt_connection);
		if (queue_puts(&clientcon->msgQueue,
				clientcon->mqtt_state.outbound_message->data,
				clientcon->mqtt_state.outbound_message->length) == -1) {
			MQTT_ERROR("MQTT: Queue full\r\n");
...
static void ICACHE_FLASH_ATTR mqtt_client_discon_cb(void *arg) {
	struct espconn *pCon = (struct espconn *) arg;
	MQTT_ClientCon *clientcon = (MQTT_ClientCon *) pCon->reverse;
...
static void ICACHE_FLASH_ATTR mqtt_client_con_sent_cb(void *arg) {
	struct espconn *pCon = (struct espconn *) arg;
	MQTT_ClientCon *clientcon = (MQTT_ClientCon *) pCon->reverse;
...
static void ICACHE_FLASH_ATTR mqtt_client_connected_cb(void *arg) {
	...
	espconn_regist_sentcb(pespconn, mqtt_client_con_sent_cb);
	espconn_regist_disconcb(pespconn, mqtt_client_discon_cb);
	espconn_regist_recvcb(pespconn, mqtt_client_connect_recv_cb);
	...
	if (no_mem) {MQTT_ERROR("ERROR: No mem for new client connection\r\n");}
	...
	mqtt_server_init_client_connect(mqttClientCon);
...
void ICACHE_FLASH_ATTR mqtt_server_task(os_event_t * event) {
	MQTT_ClientCon *clientcon = (MQTT_ClientCon *) event->par;
	...
	case MQTT_DATA:
		if (!queue_is_empty(&clientcon->msgQueue) && clientcon->sendTimeout == 0
				&& queue_gets(&clientcon->msgQueue, dataBuffer, &dataLen,
				MQTT_BUF_SIZE) == 0) {

			clientcon->mqtt_state.pending_msg_type = mqtt_get_type(dataBuffer);
			clientcon->mqtt_state.pending_msg_id = mqtt_get_id(dataBuffer,
					dataLen);
...
//少数客户端（至少8个已经过测试，内存是问题）
bool ICACHE_FLASH_ATTR mqtt_server_start(uint16_t port_no, uint16_t max_subscriptions, uint16_t max_retained_topics) {
	...
	pCon->proto.tcp->local_port = port_no;

	/* Register callback when clients connect to the server */
	espconn_regist_connectcb(pCon, mqtt_client_connected_cb);

	/* Put the connection in accept mode */
	espconn_accept(pCon);

	bool ret = system_os_task(mqtt_server_task, MQTT_SERVER_TASK_PRIO, mqtt_procServerTaskQueue, MQTT_TASK_QUEUE_SIZE);
	return ret;
}
...
bool ICACHE_FLASH_ATTR mqtt_local_subscribe(uint8_t * topic, uint8_t qos) {
	return add_topic(LOCAL_MQTT_CLIENT, topic, qos);
}
